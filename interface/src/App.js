...
function App() {
  ...
  useEffect(() => {
    ...
    const checkBackendStatus = async () => {
      try {
        await axios.get('http://192.168.1.100:8000/api/system-stats');
  ...
  useEffect(() => {
    if (backendStatus === 'connected') {
      ...
      axios.get('http://192.168.1.100:8000/api/items')
  ...
  const addItem = () => {
    axios.post('http://192.168.1.100:8000/api/items', newItem)
      .then(response => {
  ...
  return (
    <WebSocketProvider url="ws://192.168.1.100:8000/ws">
      <div>
